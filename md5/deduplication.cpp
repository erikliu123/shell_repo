...
uint64_t start_lba = 1000, lba_count = 1024;
char zero_buf[4096];
bool g_accel = true;

void help(char *app)
{
    printf("%s: /dev/nvme 1(accel)", app);
    return ;
}
struct  md5_key
{
...
int main(int argc, char **argv)
{
        ...
        md5_key result;
         if(g_accel){
            ...
            //printf("consume time: %lu\n", consume_time);
             for(uint64_t i=0; i<lba_count; i++ ){
                ...
                if(md5_digest_arr[i] == md5_zero || md5_digest_arr[i].md5[0]==0)
                    continue;
                if(md5_table.count(md5_digest_arr[i])){//TODO: caculate time
            ...
            for(uint64_t i=0; i<lba_count; i++ ){
                ...
                if(result == md5_zero)
                    continue;
                if(md5_table.count(result)){//TODO: caculate time
    ...
    printf("md5 table size:%lu, deuplicates: %d, duplicate rate: %.2f, IO: %.2f, TOTAL: %.2f\n", md5_table.size(), deplicate_blocks, 1.0*deplicate_blocks /(times*lba_count), iotime, timeuse);
    return 0;
